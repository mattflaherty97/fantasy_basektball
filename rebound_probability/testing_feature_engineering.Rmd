---
title: "testin_feature_engineering"
...
testing_data_pbp <- testing_data_pbp %>%
  ...
  select(-off_rebs, - off_rebchances, -games, -minutes) %>%
...
# Remove unnecessary variables from pbp test
...
```{r}
testing_data_pbp <- testing_data_pbp %>%
  select(-row_type, -off_team_id, -def_team_id, -home_team_id, -away_team_id, -eventdescription, -actiondescription, -playerid_off_player_1:-playerid_def_player_5, -shooter_player_id)
```
...
```{r}
test_sub_loc <- testing_data_loc %>%
  filter(as.numeric(game_id) <= 7689)

test_sub_pbp <- testing_data_pbp %>%
  filter(as.numeric(game_id) <= 7689)
```
...
test_sub_pbp <- test_sub_pbp %>%
  left_join(test_sub_loc, by = "playbyplayorder_id")

test_sub_pbp <- test_sub_pbp %>%
  select(-game_id.x,-game_id.y, -row_type, -secondchance, -andone, -inbonus)
```
...
```{r}
test_sub_pbp$off_reb1 <- log(test_sub_pbp$off_reb1)
test_sub_pbp$off_reb2 <- log(test_sub_pbp$off_reb2)
test_sub_pbp$off_reb3 <- log(test_sub_pbp$off_reb3)
test_sub_pbp$off_reb4 <- log(test_sub_pbp$off_reb4)
test_sub_pbp$off_reb5 <- log(test_sub_pbp$off_reb5)


test_sub_pbp$def_reb1 <- log(test_sub_pbp$def_reb1)
test_sub_pbp$def_reb2 <- log(test_sub_pbp$def_reb2)
test_sub_pbp$def_reb3 <- log(test_sub_pbp$def_reb3)
test_sub_pbp$def_reb4 <- log(test_sub_pbp$def_reb4)
test_sub_pbp$def_reb5 <- log(test_sub_pbp$def_reb5)
